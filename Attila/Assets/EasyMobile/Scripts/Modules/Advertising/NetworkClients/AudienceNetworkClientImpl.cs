...
namespace EasyMobile
{
    ...
    public class AudienceNetworkClientImpl : AdClientImpl
    {
        ...
        private static string GetHashedIdIOS()
        {
#if UNITY_IOS
            string testDeviceID = "";
            testDeviceID = UnityEngine.iOS.Device.advertisingIdentifier;
                testDeviceID = CreateSHA1(testDeviceID);
                testDeviceID = testDeviceID.ToLower();
       
                int length = adSettings.TestDevices.Length;
                string[] tempDeviceIds = new string[length + 1];
                if (length > 0)
                {
                    for (int i = 0; i < length; i++)
                    {
                        tempDeviceIds[i] = adSettings.TestDevices[i];
                    }
                }
                tempDeviceIds[length] = testDeviceID;
                adSettings.TestDevices = tempDeviceIds;
            return testDeviceID;
#else
            return null;
#endif
        ...
        private static string CreateSHA1(string input)
        {
