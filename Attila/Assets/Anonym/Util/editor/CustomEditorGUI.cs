    ...
    public partial class CustomEditorGUI
    {
        ...
        public class SimpleGrid<T> where T : Object
        {
            ...
            public void Init(string name, float fSize, 
                ...
                bool selectable = true, bool objectField = true, bool multiSelectable = true)
            {
                fieldName = name;
                // action = act;
                BtnName = btnName;
            ...
            public static bool ShowGrid(string fieldName, System.Action<T, Rect> action,
                ...
                List<T> selection, bool bObjectField, bool bMultiSelectable, Color selectedColor, bool isFoldable = true)
            {
                ...
                int xCount = Mathf.Max(1, Mathf.FloorToInt(fViewWidth / (fCellSize + 4.4f))), xTmp = 0;
                ...
                if (bFoldOUt)
                {
                    float fMaxHeight = (yCount + 0.25f) * fCellSize;
                            ...
                            using (new EditorGUILayout.HorizontalScope())
                            {
                                for (int i = 0; i < xCount; ++i)
                                {
                                    if (bContinue = i == 0 ? true : enumerator.MoveNext())
                                    {
                                        ...
                                        {
                                            if (current != null)
                                                bResult |= DrawCell(action, current, selection, fCellSize, bObjectField, bMultiSelectable, selectedColor);
                                        }
                                        else if (i == 0)
