...
{	
    ...
    public class IsoTile : MonoBehaviour
    {
        ...
        public void Copycat(IsoTile from, bool bCopyBody, bool bCopyChild = true, bool bUndoable = true, string undoName = "IsoTile:Copycat")
        {
            copycat(from, bCopyBody, bCopyChild, bUndoable, undoName);
        }

        void copycat(IsoTile from, bool bCopySide = true, bool bCopyAttachment = true, bool bUndoable = true, string undoName = "IsoTile:Copycat")
        {
            if (from == null || from == this || !(bCopySide || bCopyAttachment))
                return;
            ...
            foreach (Transform child in from.transform)
            {
                if (KeepOrKick(child.gameObject, bCopySide, bCopyAttachment))
                {
                    ...
                    newList.Add(newOne);
                    UndoUtil.Create(newOne, undoName);
                }
            }
            ...
            for (int i = transform.childCount - 1; i >= 0; --i)
            {
                ...
                //if (!KeepOrKick(current, bCopySide, bCopyAttachment))
                //    continue;
            ...
            ChangeTileSet(from.tileSetSprites, false, true, undoName);
...
#region MonoBehaviour
        ...
        public static bool Create(ref GameObject target, Vector3 position, IsoTile refTile, IsoTileBulk bulk,
            bool bIncludeBody, bool bIncludeAttachments, bool bAutoIsoLight, bool bNewPrefabStyle = false)
        {
        ...
        public static bool CreateORCopy(ref GameObject target, Vector3 vAt, IsoTile refTile, IsoTileBulk refBulk, 
            bool bBody, bool bAttachments, bool bAutoIsoLight, bool bNewPrefabStyle, bool bAutoCreation)
        {
            ...
            if (!bTileAtPosition || target == null)
            {
                ...
                if (Create(ref target, vAt, refTile, refBulk, bBody, bAttachments, bAutoIsoLight, bNewPrefabStyle))
                {
